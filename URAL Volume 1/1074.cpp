...
class Application{
      ...
      bool simple_unsigned_real_number(const string& s){
           ...
           if (s.substr(0,1)=="." &&unsigned_integer_number(s.substr(1,s.length()-1)))
              return true;
      ...
      bool simple_real_number(const string& s){
           ...
           for (int i=1;i<s.length();i++)
               if (sign(s.substr(0,i)) &&simple_unsigned_real_number(s.substr(i,s.length()-i)))
               return true;
           return false;
      }
      ...
      void print_real_number(const string& s,int N){
           ...
           else integral_part=simple_real_number_part;
           ...
           if (value_of_e<0){
              fractional_part=integral_part.substr(integral_part.length()+value_of_e,-value_of_e)+fractional_part;
              integral_part.erase(integral_part.length()+value_of_e,-value_of_e);
           }
           if (value_of_e>0){
              integral_part=
              integral_part
